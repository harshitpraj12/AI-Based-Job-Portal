...
import { 
  Briefcase, LayoutDashboard, Building2, User, Settings, Bell, 
  Sun, Moon, LogOut, Menu, X, PlusCircle, Users 
} from 'lucide-react';
...
const RecruiterLayout = () => {
  ...
  const navItems = [
    ...
    { name: 'Applicants', path: '/recruiter/applications', icon: Users },
        ...
        <nav className="flex-1 space-y-1 px-4 py-4 overflow-y-auto">
          ...
          <div className="pt-4 px-2">
            <Link
              ...
              className="flex items-center justify-center space-x-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500 hover:shadow-indigo-100 transition dark:bg-indigo-500 dark:hover:bg-indigo-400"
            >
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="flex h-16 items-center justify-between px-6 border-b border-gray-200/80 bg-white/70 backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-900/70 z-30">
          ...
          <div className="hidden lg:block text-sm text-gray-500 dark:text-gray-400 font-medium">
            Recruiting Portal &mdash; <span className="font-semibold text-gray-800 dark:text-white">{user?.name}</span>
          </div>
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        <div className="text-xs">
                          <p className="font-medium text-gray-900 dark:text-white">New Application Submitted</p>
                          <p className="text-gray-500">Jane Doe applied for Senior Frontend Engineer.</p>
                          <span className="text-[10px] text-indigo-500 font-semibold">5 minutes ago</span>
                        </div>
                      </div>
                    </motion.div>
                  </>
                  ...
                  <>
                    ...
                    >
                      <Link
                        ...
                        className="flex w-full items-center px-3 py-2 text-sm text-gray-700 rounded-lg hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-slate-800"
                        onClick={() => setUserMenuOpen(false)}
                      >
                        <User className="mr-2 h-4 w-4 text-gray-550" /> Profile
                      </Link>
                      <button
                        onClick={handleLogout}
                        className="flex w-full items-center px-3 py-2 text-sm text-red-650 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 cursor-pointer"
                      >
